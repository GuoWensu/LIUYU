...
int Backtrack(int r)//回溯函数
{

	if (r >= goods.m) //如果搜索到叶子结点
	{   

		if (cp >= bestp)				//如果当前最大价值大于上一次最优价值
		{   
			bestp = cp;    			//则当前物品最大价值为最优价值
		...
		for (i = 0; i < goods.n; i++)
		{
			now_add[i] = now_add[i] + goods.a[i][r];       //表示把该物品属性值加到原本背包的属性中，当前属性值总和加上下一个属性值
		} 
